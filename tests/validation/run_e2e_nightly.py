#!/usr/bin/env python3
"""Generic E2E launcher for the PyTorch nightly ROCm 7.14 validation.

Prints the no-workaround validation context, then runs the target upstream
SenseNova-U1 example exactly as scripts/run-task.sh would (same flags),
but under the nightly interpreter.

Usage:
  SENU15_MIOPEN=1 PYTHONPATH=<nightly-sp>:<repo-sp>:<upstream-src> \
    <nightly-python> run_e2e_nightly.py <upstream example>/inference.py [flags...]
...
if len(sys.argv) < 2:
    print("usage: run_e2e_nightly.py <script.py> [args...]", file=sys.stderr)
    sys.exit(2)
