...
### Added
- Upstream nightly follow-up (SDPA / t2i, 2026-08-26): **pytorch#194498's fused-SDPA launch failures are absent on the upstream nightly packaging** (torch 2.15.0.dev20260825+rocm7.14 bundles `aotriton.images/` + `libaotriton_v2.so` in the wheel — the missing-images defect of AMD leaf wheels cannot occur): fresh-process matrix phase C 11/11 (FLASH 4/4, EFFICIENT 4/4, MATH 3/3), fused-vs-MATH norm_rel ≤ 5.4e-6 with median_rel 0, MATH sums bit-identical across phases A/B/C. **NEW finding — nightly t2i/generation silently corrupts**: exits 0 but emits a washed-out repeating grid texture; reproduced at 2048@50 and 1024@25; identical config on the torch 2.8 stack renders correctly (hcorr 0.988 vs 0.739); bit-identical corruption with fused SDPA disabled (fused kernels exonerated; stock dispatcher already MATH for these layouts); nightly VQA remains correct — generation-path-specific, op not yet bisected, nightly NOT recommended for generation. Finding: docs/results/findings/pytorch-nightly-rocm714-sdpa-t2i.md; receipts: docs/results/validation/sdpa-gfx11/{phaseC-nightly-matrix.json, phaseC-cases.tar.gz, pip-freeze-nightly-rocm714.txt, t2i-nightly-receipt.json}; transcripts: docs/results/findings/transcripts/sdpa-nightly-rocm714/ (6 files); repro: tests/validation/{run_e2e_nightly.py, run_e2e_nightly_mathonly.py, sdpa_fused_vs_math_nightly.py}; scripts/sdpa-gfx11-matrix.sh now records the real torch version in matrix.json (was dead-code null).
